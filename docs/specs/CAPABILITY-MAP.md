...
| Campo  | Valor                                                                                               |
| ------ | --------------------------------------------------------------------------------------------------- |
| Origem | [PRD §6, §9–13](../PRD.md), [ADR-001](../decisions/ADR-001-stack-frontend-cache-estado-e-deploy.md) |
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | ------------------------------------ |
| `foundation`          | Esqueleto Next.js (App Router, TS), grupos de rotas e layouts `(vitrine)` / `(conta)` / `admin`, páginas de sistema (404, 500, manutenção), rota de healthcheck, clientes Supabase (servidor/navegador) e tooling de migrações, variáveis de ambiente, lint/format/test, Docker `standalone` + proxy TLS, CI de build/push/deploy no VPS, base de observabilidade (erros, helper de eventos de analytics), tokens de design provisórios (paleta neutra) | —                                              | §8, §14, ADR-001                     |
...
| ---- | ---------------------------------------------------------------------------------- |
| 1    | Vitrine no VPS respondendo HTTPS com 404 da marca; CI verde; Supabase conectado    |
...
- **Identidade visual**: paleta e tipografia estão indefinidas (PRD §15). `foundation` entrega tokens provisórios neutros em variáveis CSS; a troca pela paleta final é uma mudança de tokens, não de componentes.
...
| ---------------------------------------- | ------------------------------------------------------------------------------------ |
| `foundation`                             | Projeto Supabase ✔, VPS ✔, repositório remoto (a criar), domínio apontado para o VPS |
