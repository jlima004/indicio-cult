...
**Descrição:** Seguindo `deploy/README.md`: preparar o VPS (usuário, Docker, firewall, chave SSH dedicada à CI, `/srv/indicio-cult` com `.env` completo — Supabase, `SITE_HOST=<ip-com-hifens>.sslip.io`, `APP_VERSION` é injetado pela imagem, `SENTRY_DSN` se já existir — mais `docker-compose.yml` e `Caddyfile`); cadastrar em *Settings → Secrets*: `VPS_HOST`, `VPS_USER`, `VPS_SSH_KEY`, `SITE_HOST`. Pode começar logo após T1, com o `deploy/README.md` ainda em rascunho se a operadora preferir.
